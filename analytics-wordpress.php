<?php
/*
Plugin Name: Analytics for WordPress — by Segment.io (with L&W blog events)
...
class Segment_Analytics_WordPress {
  ...
  private function get_current_page_track() {
    ...
    if ( $settings['track_posts'] ) {
      ...
      if ( is_single() && ! is_attachment() ) {
        ...
        if ( ! self::is_excluded_post_type() ) {
          ...
          $track = array(
            'event'      => sprintf( __( 'Viewed Blog %s', 'segment' ), ucfirst( get_post_type() ) ),
            'properties' => array(
    ...
    if ( $settings['track_pages'] ) {
      ...
      if ( is_front_page() ) {
        $track = array(
          'event' => __( 'Viewed Blog Home Page', 'segment' )
        );
      }
      // A normal WordPress page.
      else if ( is_page() ) {
        $track = array(
          'event' => sprintf( __( 'Viewed Blog %s Page', 'segment' ), single_post_title( '', false ) ),
        );
      }
    }
    ...
    if ( $settings['track_archives'] ) {
      ...
      $track  = array(
        'event'      => __( 'Viewed Blog Author Page', 'segment' ),
        'properties' => array(
      ...
      // http://codex.wordpress.org/Function_Reference/single_tag_title
        ...
        $track = array(
        'event'      => __( 'Viewed Blog Tag Page', 'segment' ),
        'properties' => array(
      ...
      // http://codex.wordpress.org/Function_Reference/single_cat_title
        ...
        $track = array(
        'event'      => __( 'Viewed Blog Category Page', 'segment' ),
        'properties' => array(
    ...
    if ( $settings['track_login_page'] ) {
      ...
      if ( did_action( 'login_init' ) ) {
        ...
        $track = array(
          'event'      => __( 'Viewed Blog Login Page', 'segment' )
        );
    ...
    if ( $settings['track_searches'] ) {
      ...
      if ( is_search() ) {
        $track = array(
          'event'      => __( 'Viewed Blog Search Page', 'segment' ),
          'properties' => array(
